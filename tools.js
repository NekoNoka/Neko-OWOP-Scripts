...
(function () {
  ...
  */
  ...
  OWOP.tool.addToolObject(new OWOP.tool.class('Polybius Paste', OWOP.cursors.paste, OWOP.fx.player.NONE, OWOP.RANK.USER, tool => {
    ...
    function tick() {
      for (let i = 0; i < p2.length; i++) {
        ...
        if (OWOP.world.setPixel(pix[0], pix[1], pix[2], false)) {
          p2.splice(i--, 1);
        }
      }
    ...
    tool.setEvent('mousedown', function (p6) {
      if (p6.buttons & 1) {
        if (p2.length) return;
